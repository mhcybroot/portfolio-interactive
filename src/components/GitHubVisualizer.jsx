    ...
    <section id="repositories" className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
          ...
          >
            ...
            <div>
                ...
                <div className="flex items-center gap-2">
                  {repo.liveUrl && (
                    <a
                      ...
                      className="p-1 rounded-md bg-[#080d1a] border border-[#1f2e4d] text-purple-400 hover:text-white text-xs flex items-center gap-1 px-2"
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="p-1.5 rounded-lg bg-[#080d1a] border border-[#1f2e4d] text-slate-400 hover:text-white transition-colors"
                  >
              ...
              <div className="flex flex-wrap gap-1.5 mb-6">
                {repo.techBadges.slice(0, 4).map((badge, idx) => (
                  <span
                    key={idx}
                    className="px-2 py-0.5 rounded-md bg-[#080d1a] border border-[#1f2e4d]/70 text-[11px] font-mono text-slate-300"
                  >
                    {badge}
                  </span>
                ))}
                {repo.techBadges.length > 4 && (
                  <span className="px-2 py-0.5 rounded-md bg-[#080d1a] border border-[#1f2e4d]/70 text-[11px] font-mono text-slate-500">
                    +{repo.techBadges.length - 4}
                  </span>
                )}
              </div>
            ...
            <div className="pt-4 border-t border-[#1f2e4d]/60 flex items-center justify-between gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-[#080d1a] border border-[#1f2e4d] hover:border-[#00f5d4]/40 text-[11px] font-mono text-slate-300 transition-colors"
      ...
      {activeRepo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="w-full max-w-3xl bg-[#0d1527] border border-[#1f2e4d] rounded-2xl shadow-2xl shadow-[#00f5d4]/10 overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {activeRepo.keyModules.map((mod, idx) => (
                    <div
                      key={idx}
                      className="p-3 rounded-xl bg-[#080d1a] border border-[#1f2e4d] flex flex-col sm:flex-row sm:items-center justify-between gap-2"
                    >
              ...
              {activeRepo.architecture && (
                <div>
                  ...
                  <div className="p-4 rounded-xl bg-[#080d1a] border border-[#1f2e4d] space-y-2 font-mono text-xs text-slate-300">
              ...
              <div>
                ...
                <div className="flex items-center justify-between p-3 rounded-xl bg-[#080d1a] border border-[#1f2e4d]">
                  <code className="text-xs font-mono text-emerald-400 overflow-x-auto">
                    {activeRepo.cloneCmd}
                  </code>
                  <button
                    onClick={() => handleCopyClone(activeRepo)}
                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-[#121d36] hover:bg-[#1f2e4d] text-xs font-mono text-slate-200 transition-colors ml-3 whitespace-nowrap"
                  >
            ...
            {/* Modal Footer */}
            <div className="bg-[#121d36] px-6 py-3 border-t border-[#1f2e4d] flex items-center justify-between">
              {activeRepo.liveUrl ? (
                <a
